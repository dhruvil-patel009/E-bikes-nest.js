    ...
    (async () => {
      try {
        ...
        pr.on("paymentmethod", async (ev) => {
          setLoading(true);
          try {
            // Confirm the PaymentIntent without handling next actions yet.
            ...
            // after creating paymentRequest
            const result = await pr.canMakePayment();
            console.log("pr.canMakePayment2() =>", result); //
            // If further authentication is required, handle it
            if (paymentIntent && paymentIntent.status === "requires_action") {
              const next = await stripe.confirmCardPayment(json.clientSecret);
              if (next.error) {
                alert("Authentication failed: " + next.error.message);
                setLoading(false);
                return;
